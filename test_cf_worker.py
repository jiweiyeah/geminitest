...
# -*- coding: utf-8 -*-
"""
裁判文书批量处理工具 - Cloudflare Worker 版本
批量处理Excel中的裁判文书并输出结构化数据
使用 Cloudflare Worker 作为代理调用 Gemini API
"""
...
# --- API配置 ---
CF_WORKER_URL = 'https://geminiapi.freeourdays.com'
AUTH_KEY = '123456'
...
INPUT_EXCEL_PATH = file_path + '/textExcel10.xlsx'
...
def process_row_sync(index, row_data, system_prompt, user_prompt_template, UPSTREAM_CRIME_TYPES_PROMPT):
    """
    处理单行数据的函数，包含自动重试逻辑
    使用Cloudflare Worker作为代理调用Gemini API
    """
    ...
    api_url = f"{CF_WORKER_URL}/{MODEL_NAME}"
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if not model_response_text:
                raise Exception("模型返回了空内容或非预期的格式")
            
            # 清理并解析JSON结果
            cleaned_response = model_response_text.strip().lstrip('```json').rstrip('```')
            parsed_json = json.loads(cleaned_response)
            return parsed_json

        except json.JSONDecodeError as e:
            return {"error": f"JSON解析失败: {e}", "raw_response": model_response_text if 'model_response_text' in locals() else "No response text available"}
...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=CONCURRENT_LIMIT) as executor:
        future_to_index = {executor.submit(process_row_sync, index, row, system_prompt, user_prompt_template, UPSTREAM_CRIME_TYPES_PROMPT): index for index, row in df.iterrows()}
